...
onShow(() => {
  if (memberStore.profile) getCartData()
})
...
const selectedAll = () => {
  ...
  putMemberCartCheckAllApi({ selected: _isSelectedAll })
}
const allPrice = computed(() => {
  return cartList.value.reduce((p, c) => p + (c.selected ? c.price * c.count : 0), 0).toFixed(2)
})
const selectGoosCount = computed(() => {
  return cartList.value.reduce((p, c) => (c.selected ? p + c.count : p), 0)
})
const goPayment = () => {
...
onLoad(() => {
  ...
  if (currentPath !== 'pages/cart/cart') {
    // 获取屏幕边界到安全区域距离
    const { safeAreaInsets } = uni.getSystemInfoSync()
    safeBottom.value = safeAreaInsets?.bottom + 'px'
  }
})
</script>
...
<template>
  ...
  >
    ...
    <template v-if="memberStore.profile">
      ...
      <view class="cart-list" v-if="cartList.length">
        ...
        <uni-swipe-action>
          ...
          <uni-swipe-action-item v-for="item in cartList" :key="item.skuId" class="cart-swipe">
            ...
            <!-- 右侧删除按钮 -->
            <template #right>
              <view class="cart-swipe-right" @tap="deleteCart(item.skuId)">
      ...
      <!-- 吸底工具栏 -->
      <view class="toolbar" :style="{ bottom: safeBottom }" v-show="cartList.length">
        ...
        <view class="button-grounp">
          <view
            ...
            :class="{ disabled: selectGoosCount === 0 }"
          >
            去结算({{ selectGoosCount }})
          </view>
        </view>
